...
ROCK = [0, 0, 0, 0, 0]  # Array của búa, tất cả các ngón tay đều xếp xuống
SCISSORS = [0, 1, 1, 0, 0]  # Array của kéo, ngón tay cái và ngón trỏ duỗi ra, các ngón khác xếp xuống
PAPER = [1, 1, 1, 1, 1]  # Array của giấy, tất cả các ngón tay đều duỗi ra
...
while True: #tạo một vòng lặp vô hạn để chưởng trình chạy liên tục và xử lý liên tục các khung hình video
    ...
    if hands: # kiểm tra xem bàn tay đã được phát hiện hay chưa
        for hand in hands: # dùng vòng lặp duyệt qua từng bàn tay được phát hiện
            ...
            display_text = f"{handType}: {gesture} " # Tạo string hiển thị số ngón tay được duỗi ra và kiểu bàn tay
